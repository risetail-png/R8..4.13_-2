...
def ocr_diagram_page(pdf_path, page_idx, dpi):
    ...
    タイトルに「補修箇所」が含まれる場合は None を返す（リンク対象外）。
    """
    ...
    img = imgs[0]
    w, h = img.size
    # ── タイトル領域（ページ上部 12〜28%）をOCRして「補修箇所」を検出 ──
    title_crop = img.crop((int(w * 0.15), int(h * 0.12),
                           int(w * 0.85), int(h * 0.28)))
    title_text = pytesseract.image_to_string(title_crop, lang='jpn', config='--psm 6')
    title_text = _normalize_text(title_text)
    if '補修箇所' in title_text:
        return None   # 補修箇所ページ → スキップ
    t = pytesseract.image_to_string(img, lang='jpn', config='--psm 11')
    ...
    try:
        ...
        for didx in diag_pages:
            ...
            if not pairs:
                ...
                ocr_result = ocr_diagram_page(input_path, didx, dpi)
                if ocr_result is None:
                    log_cb(f"  損傷図 p.{didx+1}: 補修箇所ページを検出 → リンク対象から除外")
                    continue   # diagram_to_photo_keys に登録しない → ボタン追加対象外
                pairs = ocr_result
